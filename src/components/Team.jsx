              ...
              {cardData && cardData.slice(startIndex, startIndex + cardRemCount).map((item, index) => (
                <div className={styles["profile-card"]} key={index}>
                  ...
                  <div className={styles["social-icons"]}>
                    <a href="#" className={styles["linkedin-icon"]}><FontAwesomeIcon className={styles['icon']} icon={faLinkedin} color="#0077B5" /></a>
                    <a href="#" className={styles["instagram-icon"]}><FontAwesomeIcon className={styles['icon']} icon={faInstagram} color="#E4405F" /></a>
                    <a href="#" className={styles["facebook-icon"]}><FontAwesomeIcon className={styles['icon']} icon={faFacebook} color="#3b5998" /></a>
                  </div>
                </div>
              ))}

            </div>
            <img className={`${styles["arrow"]} ${styles["left-arrow"]}`} onMouseEnter={() => setLeftHover(true)} onMouseLeave={() => setLeftHover(false)} onClick={handlePrevClick} src={lefthover ? arrowred : arrow}></img>
            <img className={`${styles["arrow"]} ${styles["right-arrow"]}`} onMouseEnter={() => setRightHover(true)} onMouseLeave={() => setRightHover(false)} onClick={handleNextClick} src={righthover ? arrowred : arrow}></img>
          </div>
